function [W, a, train_err] = train_foe_pcd(images, basis, no_experts, a)
%TRAIN_FOE Trains a Field of Experts model on a collection of images
%
%   [W, a, train_err] = train_foe_pcd(images, basis, no_experts)
...
%                 hmc('foe_energy', im(1:end), options, 'foe_energy_grad_x', basis, W, a, [size_im size_im 1]);
                ...
                chain(i,:) = hmc('foe_energy', chain(i,:), options, 'foe_energy_grad_x', basis, W, a, [size_im size_im 1]);
                
                % Compute negative CD part
                neg_W = neg_W + reshape(foe_energy_grad_w(W(1:end), chain(i,:), basis, a), size(W));
                neg_a = neg_a + foe_energy_grad_a(a, chain(i,:), basis, W);
            end
